...
BEGIN
    ...
    -- Technical evaluation guard gate
    IF `v_user_role` = 'Employee' THEN
        
        IF `v_difficulty` = 'High' THEN 
            SET `v_difficulty_multiplier` = 3;
        ELSEIF `v_difficulty` = 'Medium' THEN 
            SET `v_difficulty_multiplier` = 2;
        ELSE 
            SET `v_difficulty_multiplier` = 1;
        END IF;

        -- Mathematical Schedule Health Scaling Matrix
        IF NEW.`outcome_status` = 'Failed' THEN
            SET `v_calculated_score` = `v_complexity` * 2;
        
        ELSEIF NEW.`completion_status` = 'Completed_OnSchedule' AND NEW.`outcome_status` = 'Success' THEN
            SET `v_calculated_score` = (`v_complexity` * 10) * `v_difficulty_multiplier`;
        
        ELSEIF NEW.`completion_status` = 'Completed_Overdue' AND NEW.`outcome_status` = 'Success' THEN
            SET `v_calculated_score` = ((`v_complexity` * 10) * `v_difficulty_multiplier`) * 0.80;
        
        ELSEIF NEW.`completion_status` = 'InProgress_OnSchedule' THEN
            SET `v_calculated_score` = ((`v_complexity` * 10) * `v_difficulty_multiplier`) * 0.50;
        
        ELSEIF NEW.`completion_status` = 'InProgress_Overdue' THEN
            SET `v_calculated_score` = ((`v_complexity` * 10) * `v_difficulty_multiplier`) * 0.20;
        ELSE
            SET `v_calculated_score` = 0.00;
        END IF;
...
-- 2. Populate Corporate Time-Bound Cycles
INSERT INTO `appraisal_periods` (`id`, `period_name`, `start_date`, `end_date`) VALUES
(1, '2025_H1', '2025-01-01', '2025-06-30'),
(2, '2025_H2', '2025-07-01', '2025-12-31'),
(3, '2026_Q1', '2026-01-01', '2026-03-31');

-- 3. Populate Employees Dataset
...
(8, 'Tariq', 'Mansoor', 't.mansoor@company.com', 'tmansoor', 'tariq2026', 'Employee', 'Associate Researcher', 3, NULL, '2025-01-10');

-- 4. Populate Projects Strategic Roadmap Portfolio
...
-- 5. Map Assignment Relationships
INSERT INTO `project_assignments` (`employee_id`, `project_id`) VALUES
(4, 1), (4, 2), (5, 4), (6, 5), (6, 3), (7, 6), (7, 1), (8, 7);

-- 6. Seed Multi-Cycle Evaluation Transactions
...
(4, 1, 1, 'InProgress_OnSchedule', 'N/A', 'Alice tracking on target for initial lifecycle framework verification lines.'),
(4, 1, 2, 'InProgress_Overdue', 'N/A', 'Execution pace bottlenecked by environmental blocks and resource constraints.'),
...
-- VIEWS
-- ============================================================================
-- View 1: Functional Itemized Profile 
...
-- View 2: Corporate Summary Execution View (Aggregated performance profile lines)
...
GROUP BY vips.`appraisal_period_id`, vips.`employee_name`;

-- View 3: Portfolio Asset Delivery Health Report for Managers & HR
...
BEGIN
    ...
    IF `v_user_role` IN ('HR', 'Manager') THEN
        SELECT `period_name`, `department_name`, `project_name`, `difficulty`, `completion_status`, `outcome_status`, `team_average_performance_score`, `allocated_staff_count`
...
-- Executive Master Performance Control 
